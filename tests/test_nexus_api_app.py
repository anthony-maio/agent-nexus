...
def test_skill_setup_steps_seed_run_with_rendered_external_tool_preflight(tmp_path: Path) -> None:
    skill_root = tmp_path / "skills"
    _write_skill(
        skill_root,
        "memory-helper",
    ...
    first_step = run["steps"][0]
    assert first_step["action_type"] == "external_tool"
    assert first_step["metadata"]["planner_source"] == "skill"
...
def test_kernel_completion_recovery_uses_cartograph_for_stalled_coding_runs(tmp_path: Path) -> None:
    script_path = _write_fake_stdio_mcp_server(tmp_path)
    tool_config = json.dumps(
        [
            {
    ...
    assert repo_step["metadata"]["external_tool"]["name"] == "cartographer.map_repo"
    assert repo_step["metadata"]["tool_result"]["echo"]["objective"] == (
        "Implement retry backoff in the repo"
    )
    verification = run["metadata"]["run_verification"]
    assert verification["result"] == "provisional"
